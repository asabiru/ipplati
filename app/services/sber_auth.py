...
from app.models import BankTransaction, SberToken, TransactionDirection
...
def build_state() -> str:
    ...
    signature = hmac.new(
        settings.sber_state_secret.encode("utf-8"),
        nonce.encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
...
def build_authorization_url() -> str:
    if not settings.sber_client_id:
        raise ValueError("SBER_CLIENT_ID is not configured")
    ...
    return f"{settings.sber_oauth_authorize_url}?{urlencode(params)}"


def _build_ssl_context() -> ssl.SSLContext | bool:
    if settings.sber_verify_tls:
        if settings.sber_tls_ca_chain_path:
            context = ssl.create_default_context(cafile=settings.sber_tls_ca_chain_path)
        else:
            context = ssl.create_default_context()
    else:
        context = ssl._create_unverified_context()

    if settings.sber_tls_cert_path and settings.sber_tls_key_path:
        context.load_cert_chain(settings.sber_tls_cert_path, settings.sber_tls_key_path)
        return context

    if settings.sber_tls_p12_path:
        cert_path, key_path = _extract_p12_to_pem(
            settings.sber_tls_p12_path,
            settings.sber_tls_p12_password or None,
        )
...
def _extract_p12_to_pem(p12_path: str, password: str | None) -> tuple[str, str]:
    source = Path(p12_path)
    if not source.exists():
        raise FileNotFoundError(f"Sber TLS certificate file not found: {source}")
    ...
    if private_key is None or certificate is None:
        raise ValueError("Sber TLS certificate archive does not contain a private key and certificate")

    fingerprint = hashlib.sha256(raw).hexdigest()[:16]
    base_dir = Path(tempfile.gettempdir()) / "ip-automation-sber"
    base_dir.mkdir(parents=True, exist_ok=True)
    cert_path = base_dir / f"sber-client-{fingerprint}.crt.pem"
    key_path = base_dir / f"sber-client-{fingerprint}.key.pem"

    cert_bytes = certificate.public_bytes(Encoding.PEM)
    if additional_certs:
        cert_bytes += b"".join(extra.public_bytes(Encoding.PEM) for extra in additional_certs)
...
def save_token(session: Session, payload: dict) -> SberToken:
    ...
    token = session.scalar(select(SberToken).order_by(SberToken.obtained_at.desc()))
    if token is None:
        token = SberToken(access_token=payload["access_token"])
        session.add(token)
...
def exchange_authorization_code(code: str, session: Session) -> SberToken:
    if not settings.sber_client_secret:
        raise ValueError("SBER_CLIENT_SECRET is not configured")
    ...
    with build_http_client() as client:
        response = client.post(settings.sber_oauth_token_url, data=form_data, headers=headers)
        response.raise_for_status()
        payload = response.json()
    return save_token(session, payload)


def refresh_access_token(session: Session) -> SberToken:
    token = session.scalar(select(SberToken).order_by(SberToken.obtained_at.desc()))
...
def get_current_token(session: Session) -> SberToken | None:
    return session.scalar(select(SberToken).order_by(SberToken.obtained_at.desc()))


def get_valid_access_token(session: Session) -> str:
    token = get_current_token(session)
    if token is None:
        raise ValueError("No Sber token is stored")
...
def fetch_user_info(session: Session) -> dict:
    ...
    with build_http_client() as client:
        response = client.get(settings.sber_oauth_userinfo_url, headers=headers)
        response.raise_for_status()
        raw = response.text
    payload = decode_jwt_without_verification(raw)
    return payload or {"raw": raw}
...
def _extract_amount(transaction: dict) -> Decimal:
    ...
    if raw is None:
        raise ValueError(f"Transaction {transaction.get('operationId') or transaction.get('uuid')} has no amount")
...
def _extract_booked_at(transaction: dict) -> datetime:
    ...
    if raw is None:
        raise ValueError(f"Transaction {transaction.get('operationId') or transaction.get('uuid')} has no date")
    if "T" in raw:
...
def _map_statement_transaction(transaction: dict) -> dict:
    ...
    payer_name = transfer.get("payerName")
    payer_account = transfer.get("payerAccount")
    ...
    return {
        "external_id": str(
            transaction.get("operationId")
            or transaction.get("uuid")
            or transaction.get("hashAbc")
...
def fetch_statement_transactions(session: Session, account_number: str, statement_date: str, page: int = 1) -> dict:
    ...
    with build_http_client() as client:
        response = client.get(settings.sber_statement_transactions_url, params=params, headers=headers)
        response.raise_for_status()
