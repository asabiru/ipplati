...
- `/kudir` - КУДиР (книга учёта доходов и расходов) в формате Excel
- `/help` - список команд
...
Генерация КУДиР для ИП на УСН (доходы − расходы). Берет данные из банковских транзакций и формирует Excel-файл по стандартной форме.

Настройки в `.env`:

```env
KUDIR_IP_NAME=ИП Иванов Иван Иванович
KUDIR_INN=000000000000
KUDIR_TAX_YEAR=2025
KUDIR_TAX_SYSTEM=УСН (доходы − расходы)
KUDIR_INCOME_BASIS=Доход от оказания посреднических услуг
KUDIR_EXPENSE_THIRD_PARTY_BASIS=Расходы на оплату услуг третьих лиц, связанных с осуществлением посреднической деятельности
KUDIR_EXPENSE_BANK_FEE_BASIS=Банковская комиссия за расчетно-кассовое обслуживание (пакет переводов физическим лицам)
KUDIR_ARTIFACTS_DIR=artifacts/kudir
```

API endpoint:

```
GET /reports/kudir/xlsx?date_from=2025-12-01&date_to=2025-12-31
```

В Telegram-боте:
- Команда `/kudir` — генерирует КУДиР за текущий месяц
- Кнопка «КУДиР за месяц» / «КУДиР по датам» в меню

Структура Excel-файла:
- Шапка: название ИП, ИНН, налоговый период, система налогообложения
- Таблица: №, Дата операции, № документа (формат `DD.MM-NN`), Доход ₽, Расход ₽, Основание

Логика классификации:
- Входящие банковские операции → **доход** (посреднические услуги)
- Исходящие банковские операции → **расход** (услуги третьих лиц)
- Операции с ключевыми словами «комиссия», «РКО», «обслуживание» → **расход** (банковская комиссия)
